...
<body>
  <h2>My CD Collection</h2>
    <xsl:apply-templates>
      <xsl:with-param name="paramEspacos" select="--" />
    </xsl:apply-templates>
</body>
</html>
</xsl:template>
...
<xsl:template match="@* | node()"> <!-- no ff68,  -->
  ...
  <xsl:copy>
    <xsl:apply-templates select="@* | node()" >
      <xsl:with-param name="paramEspacos" select="concat($paramEspacos,'--')" /> <!-- a cada nível, acrescenta dois espaços quando falta o $, falha sem mensagem de erro. -->
    </xsl:apply-templates>
  </xsl:copy>
</xsl:template>
